...
a.ptags_input_box){d.val(g.text()).focus().select();a.ptags_remove(g.text())}return false}));a.ptags_editable&&a.ptags_input_box&&g.hover(function(){b(this).addClass("ui-state-hover")},function(){b(this).removeClass("ui-state-hover")});a.ptags_remover&&g.children().addClass("ui-ptags-tag-text-icon").end().append(b('<a href="#" />').addClass("ui-ptags-tag-remover ui-icon ui-icon-circle-minus").click(function(){a.ptags_remove(g.text());return false}));a.ptags_tag_container.append(g)})}};a.ptags_widget.addClass("ui-ptags");
a.wrapAll(a.ptags_widget);a.ptags_widget=a.parent();a.ptags_container.addClass("ui-ptags-tag-box-container");a.wrapAll(a.ptags_container);a.ptags_container=a.parent();if(a.ptags_input_box){var d=b(a.get(0)).clone().val("").keydown(function(c){if(c.keyCode==13&&!c.shiftKey){a.ptags_add(d.val().split(a.ptags_delimiter));d.val("");return false}}).blur(function(){if(d.val()!=""){a.ptags_add(d.val().split(a.ptags_delimiter));d.val("")}});d.attr("id")&&d.attr("id",d.attr("id")+"__ptags");d.attr("name")&&
d.attr("name",d.attr("name")+"__ptags");a.ptags_container.append(d)}if(a.ptags_current_text){var j=a.val().split(a.ptags_delimiter);b.merge(a.ptags_tags,j);a.ptags_unique_check();a.ptags_update_val()}if(a.ptags_show_tags){a.ptags_tag_container=b("<span />").addClass("ui-ptags-tag-container");a.ptags_widget.append(a.ptags_tag_container);a.ptags_update_tags()}a.change(a.ptags_sync_input);a.ptags_show_box||a.css("display","none");this.pines_tags=a});return e};b.fn.ptags.defaults={ptags_tags:[],ptags_current_text:true,
ptags_delimiter:",",ptags_show_tags:true,ptags_show_box:false,ptags_input_box:true,ptags_remover:true,ptags_editable:true}})(jQuery);
